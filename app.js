 ...
 * ============================================================
 * THIS FILE IS VERSION: v0.9.1
 * Last updated: 2026-03-13
 * ============================================================
 *
 * Author: Chris White
 * Repo:   https://github.com/chriswhite3140/class-tracker
 * Live:   https://chriswhite3140.github.io/class-tracker
 *
 * v0.9.1 - Multi-subject student detail page + updated print report
 * v0.9.0 - Added printable student report
 * v0.8.0 - Added Bulk Assess view
 * v0.7.5 - Added Numeracy Progressions toggle
 * v0.7.0 - Multi-subject dashboard, class overview grid
 * ============================================================
 */
...
const GITHUB_RAW = 'https://raw.githubusercontent.com/chriswhite3140/class-tracker/main/';
...
function renderDashboard(main) {
  ...
  function subjectCard(subj) {
    ...
    const pct = assessed.length ? Math.round(subjAchieved/assessed.length*100) : 0;
    ...
    return `<div class="card" style="cursor:pointer" onclick="cdFilters.subject='${subj}';cdFilters.year='all';cdFilters.strand='all';showView('curriculum')">
      <div style="padding:14px 16px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border)">
        ...
        ${assessed.length > 0 ? `<div style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:${col}">${pct}%</div>` : `<div style="font-size:10px;color:var(--text3)">No data</div>`}
      </div>
  ...
  main.innerHTML = `
    ...
    <div class="content">
      <div class="stats-row">
        ...
        <div class="stat-card c-gold"><div class="stat-label">Gaps</div><div class="stat-value">${gaps}</div><div class="stat-sub">emerging / at risk</div></div>
      </div>
      ${subjects.length > 0 ? `
  ...
  main.innerHTML = `
    ...
    <div class="content">
      <div class="mastery-tabs">
        ...
        <button class="mastery-tab t-nottaught ${filter==='nottaught'?'active':''}" onclick="setDetailFilter('nottaught')">· Not taught <span class="tab-count">${stats.nottaught}</span></button>
      </div>
...
function buildStudentReportBody(s, opts) {
  ...
  <div style="margin-top:30px;padding-top:10px;border-top:1px solid #ddd;font-size:9px;color:#aaa;display:flex;justify-content:space-between">
    <span>ClassTracker · chriswhite3140.github.io/class-tracker</span>
...
async function init() {
  ...
  try {
    await Promise.all([loadStudents(), loadProgress(), loadTaughtLog(), fetchAllCSVs()]);
    state.loading = false;
    renderView();
    // Show "NEW" badge if no session logged today
    checkDailyLogBadge();
    // Auto-popup if nothing logged today
    const today = new Date().toISOString().split('T')[0];
    ...
    state.loading = false;
    renderView();
    toast('Could not connect to Google Sheets. Check your connection.', 'error');
  }
}
